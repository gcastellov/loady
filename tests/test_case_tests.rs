...
fn given_test_info_when_creating_test_case_then_gets_new_instance() {
    ...
    let test_case = TestCase::<'static, TestCaseContext, EmptyData>::new(TEST_NAME, TEST_SUITE, data);
...
async fn given_test_case_without_steps_when_running_then_do_nothing() {
    ...
    let (tx_internal_step, _) = mpsc::channel::<&str>(CHANNEL_BUFFER_SIZE);
    let mut test_case = TestCase::<'static, TestCaseContext, EmptyData>::new(TEST_NAME, TEST_SUITE, data);

    _ = test_case.run(&tx_load_action, &tx_load_step, &tx_internal_step).await;
...
async fn given_test_case_without_load_steps_when_running_then_do_nothing() {
    ...
    let (tx_internal_step, _) = mpsc::channel::<&str>(CHANNEL_BUFFER_SIZE);
    let mut test_case = TestCase::<'static, TestCaseContext, EmptyData>::new(TEST_NAME, TEST_SUITE, data);
    test_case.with_step(TestStep::<'static, EmptyData>::as_clean_up(Box::new(cleanup)));
    test_case.with_step(TestStep::<'static, EmptyData>::as_warm_up(Box::new(warmup), Vec::default()));
...
async fn given_test_case_with_load_step_and_empty_stages_when_running_then_do_nothing() {
    ...
    let (tx_internal_step, _) = mpsc::channel::<&str>(CHANNEL_BUFFER_SIZE);
    let mut test_case = TestCase::<'static, TestCaseContext, EmptyData>::new(TEST_NAME, TEST_SUITE, data);
    test_case.with_step(TestStep::<'static, EmptyData>::as_load("step", Box::new(load), Vec::default()));
...
async fn given_test_case_with_load_step_and_stages_when_running_then_do_something() {
    let data = EmptyData::default();
    let (tx_load_action, mut rx_load_action) = mpsc::channel::<TestCaseContext>(CHANNEL_BUFFER_SIZE);
    let (tx_load_step, mut rx_load_step) = mpsc::channel::<TestCaseContext>(CHANNEL_BUFFER_SIZE);
    let (tx_internal_step, mut rx_internal_step) = mpsc::channel::<&str>(CHANNEL_BUFFER_SIZE);
